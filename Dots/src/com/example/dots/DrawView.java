...
public class DrawView extends View {
	 ...
	 * @param rows_length количество строк.
	 * @param columns_length количество столбцов.
	 */
	...
	public void onlyDots() {
		...
		for (int i = 0; i < rows; i++) {
			for (int j = 0; j < columns; j++) {
				...
				path.moveTo(myX, myY);
				path.addCircle(myX, myY, radius, Path.Direction.CW);
...
//		adds();
	 ...
	 * @param n1 номер точки
	 * @param n2 номер соседней точки
	 * @param key ключ связи
	 */
	public void add(int n1, int n2) {
		if (n1 == 0 || n2 == 0 ) {
			Toast.makeText(context, context.getString(R.string.must_be_more), Toast.LENGTH_SHORT).show();
			return;
		} else if (n1 > matrixSize || n2 > matrixSize) {
			Toast.makeText(context, context.getString(R.string.dot_doesnt_exist), Toast.LENGTH_SHORT).show();
			return;
		}
		...
		if (((x1 == x2 + App.step || x1 == x2 - App.step) && y1 == y2) || (y1 == y2 + App.step || y1 == y2 - App.step) && x1 == x2) {
			if (list_set.size() > 0) {
				for (Iterator<HashSet<Integer>> iter = list_set.iterator(); iter.hasNext();) {
			...
			Toast.makeText(context, context.getString(R.string.dots_must_be_different), Toast.LENGTH_SHORT).show();
		} else {
			Toast.makeText(context, context.getString(R.string.dots_must_be_near), Toast.LENGTH_SHORT).show();
		}
	}
	
	private void s(int n, HashSet<Integer> set) {
		boolean isFind;
		int count = 1;
		for (Iterator<HashSet<Integer>> iter = list_set.iterator(); iter.hasNext();) {
			isFind = false;
	...
	public void delete(int n1, int n2) {
		//TODO Доделать метод, чтобы удаляло из множества точку и связанные с ней точки
		if (n1 == 0 || n2 == 0 ) {
			Toast.makeText(context, context.getString(R.string.must_be_more), Toast.LENGTH_SHORT).show();
			return;
		}
